...
def find_sim(query_song_lyrics, query_song_name, cossim_mat, title_to_index, use_images):
  ...
  for song in data["songs"]:
        if song["title"] != query_song_name:
          ...
          cossim_score = cossim_mat[title_to_index[query_song_name], title_to_index[song['title']]]
...
def get_song_lyrics(query_song_name, use_images):
  f = open('backend/data.json') if not use_images else open('backend/data-images.json')# returns JSON object as a dictionary
  data = json.load(f)
  cossim_mat = np.load('backend/cossim_matrix.npy')
  ...
  if lowercased_query_song_name in song_titles:
    ...
    return find_sim(query_song_lyrics, data['songs'][song_index]['title'], cossim_mat, title_to_index, use_images=None) # passing corrected title (case sensitive) 
